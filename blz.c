...
int main(int argc, char **argv) {
  ...
  else if (!_stricmp(argv[1], "-en"))  { cmd = CMD_ENCODE; mode = BLZ_NORMAL; }
  else if (!_stricmp(argv[1], "-eo"))  { cmd = CMD_ENCODE; mode = BLZ_BEST; }
  else if (!_stricmp(argv[1], "-en9")) { cmd = CMD_ENCODE; mode = BLZ_NORMAL; }
  else if (!_stricmp(argv[1], "-eo9")) { cmd = CMD_ENCODE; mode = BLZ_BEST; }
...
void Title(void) {
  printf(
    "\n"
    "BLZ 1.4.1 - (c) CUE and Midnoclose 2022\n"
...
void Usage(void) {
  EXIT(
    ...
    "* this encoding is used in the DS overlay files\n"
  );
}
