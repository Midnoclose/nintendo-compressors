...
int main(int argc, char **argv) {
  ...
  else if (!_stricmp(argv[1], "-evb")) { cmd = CMD_CODE_11; vram = LZX_VRAM; }
  else if (!_stricmp(argv[1], "-ewb")) { cmd = CMD_CODE_11; vram = LZX_WRAM; }
  else if (!_stricmp(argv[1], "-evl")) { cmd = CMD_CODE_40; vram = LZX_VRAM; }
  else if (!_stricmp(argv[1], "-ewl")) { cmd = CMD_CODE_40; vram = LZX_WRAM; }
...
void Title(void) {
  printf(
    "\n"
    "LZX 1.4.1 - (c) CUE and Midnoclose 2022\n"
