...
/*--  Copyright (C) 2022 CUE and Midnoclose                                 --*/
/*--                                                                        --*/
...
int main(int argc, char **argv) {
  ...
  if (argc < 2) Usage();
  if      (!_stricmp(argv[1], "-d"))   { cmd = CMD_DECODE; }
  else if (!_stricmp(argv[1], "-evn")) { cmd = CMD_CODE_10; mode = LZS_VRAM; }
  else if (!_stricmp(argv[1], "-ewn")) { cmd = CMD_CODE_10; mode = LZS_WRAM; }
  else if (!_stricmp(argv[1], "-evf")) { cmd = CMD_CODE_10; mode = LZS_VFAST; }
  else if (!_stricmp(argv[1], "-ewf")) { cmd = CMD_CODE_10; mode = LZS_WFAST; }
  else if (!_stricmp(argv[1], "-evo")) { cmd = CMD_CODE_10; mode = LZS_VBEST; }
  else if (!_stricmp(argv[1], "-ewo")) { cmd = CMD_CODE_10; mode = LZS_WBEST; }
...
void Title(void) {
  printf(
    "\n"
    "LZSS 1.4.1 - (c) CUE and Midnoclose 2022\n"
...
/*----------------------------------------------------------------------------*/
/*--  EOF                             Copyright (C) 2022 CUE and Midnoclose --*/
/*----------------------------------------------------------------------------*/
